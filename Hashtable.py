...
class HashTable:
    ...
    def add_key_val(self,key,value):
        hashedKey =  self.GenerateHash(key)
        if self.hash_table[hashedKey] is None:
            self.hash_table[hashedKey] = Node(Data(key,value),None)
        else:
            node = self.hash_table[hashedKey]
            while node.next_node:
                node = node.next_node
            node.next_node = Node(Data(key,value),None)

    def getvalue(self,key):
        hashed_key = self.GenerateHash(key)
